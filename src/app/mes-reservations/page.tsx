    ...
    <>
      ...
      <main className="mx-auto w-full max-w-3xl flex-1 px-4 py-10">
        ...
        {bookings.length > 0 && kyc !== "verified" && (
          <div className="mt-4 flex flex-wrap items-center gap-3 rounded-2xl border border-amber-400/40 bg-amber-50 dark:bg-amber-400/15 p-4">
            ...
            {kyc !== "pending" && (
              <Link
                href="/verification"
                className="rounded-xl bg-band px-4 py-2 text-sm font-semibold text-white hover:bg-brand-800"
              >
                {t.kycBtn}
              </Link>
            )}
        ...
        {bookings.length === 0 ? (
          ...
          <div className="mt-6 space-y-4">
            {bookings.map((b) => {
              ...
              return (
                <div key={b.id} className="rounded-2xl border border-ink/10 bg-card p-5">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <Link href={`/reservation/${b.id}`} className="font-bold text-ink hover:underline">
                  ...
                  {open && (
                    <div className="mt-4 flex flex-wrap items-center gap-3 border-t border-ink/10 pt-3">
                      <span className={`text-xs ${b.refundable ? "text-green-700 dark:text-green-300" : "text-ink/50"}`}>
                      ...
                      <div className="ml-auto flex gap-2">
                        ...
                        <form action={cancelBooking}>
                          ...
                          <button className="rounded-lg border border-ink/15 px-4 py-2 text-sm font-medium text-ink hover:bg-ink/5">
                            {t.cancel}
                          </button>
                        </form>
                      </div>
